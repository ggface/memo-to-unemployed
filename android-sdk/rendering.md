...
**Choreographer** синхронизирует UI с VSync. Он получает сигнал от дисплея и вызывает doFrame() на UI thread (Looper), внутри которого происходят:
...
**VSync** — это сигнал от дисплея, который приходит с частотой обновления (например, 60Hz). Он говорит системе, когда нужно рисовать следующий кадр, чтобы избежать tearing и синхронизировать рендеринг.
